      ...
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
          ...
          >
            ...
            {/* Tooltip */}
            <div className={`
              absolute w-max ${expand ? "left-1/2 -translate-x-1/2 top-12" : "-top-12 left-0"}
            ...
            `}>
              ...
              <div className={`
                w-3 h-3 absolute bg-black rotate-45 ${expand ? 
                "left-1/2 -top-1.5 -translate-x-1/2" : 
                "left-4 -bottom-1.5"} z-50
              `}></div>
            </div>
          </div>
        </div>
        <button
  className={`flex items-center transition-all duration-200 ease-in-out mt-8
    ${expand 
      ? "bg-[#1E90FF] hover:bg-[#1A7FDD] text-white rounded-xl gap-2 px-4 py-2.5 font-medium shadow-sm"
      : "group h-10 w-10 mx-auto hover:bg-gray-200/50 dark:hover:bg-gray-700/50 rounded-lg"
    }`}
>
  <Image
    className={`transition-all ${expand ? 'w-5' : 'w-6 opacity-80 group-hover:opacity-100'}`}
    src={expand ? assets.chat_icon : assets.chat_icon_dull}
    alt="New Chat"
  />
  {expand && <span className="whitespace-nowrap">New Chat</span>}
  
  {/* Tooltip (only shown in collapsed mode) */}
  {!expand && (
    <div className="absolute left-full ml-2 px-3 py-1.5 bg-gray-800 dark:bg-gray-700 text-white text-sm rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
      New Chat
      <div className="absolute right-full top-1/2 -translate-y-1/2 w-2 h-2 bg-gray-800 dark:bg-gray-700 rotate-45" />
    </div>
  )}
</button>
